...
impl fmt::Display for HostFunctionError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            HostFunctionError::OutOfBounds { offset, length, message, function } => {
                write!(f, "[{}] Memory out of bounds at offset {} length {}: {}", function, offset, length, message)
            }
            HostFunctionError::InvalidParameter { param, value, message, function } => {
                write!(f, "[{}] Invalid parameter '{}' with value '{}': {}", function, param, value, message)
            }
            ...
            HostFunctionError::GasError { message, function, gas_requested, gas_available } => {
                match (gas_requested, gas_available) {
                    (Some(req), Some(avail)) => {
                        write!(f, "[{}] Gas error: {} (requested: {}, available: {})", function, message, req, avail)
                    }
                    _ => {
            ...
            HostFunctionError::StorageError { message, function, key } => {
                match key {
                    Some(k) => write!(f, "[{}] Storage error for key '{}': {}", function, k, message),
                    None => write!(f, "[{}] Storage error: {}", function, message),
                }
            }
            HostFunctionError::CallError { message, function, target_address } => {
                match target_address {
                    Some(addr) => write!(f, "[{}] Call error to address '{}': {}", function, addr, message),
                    None => write!(f, "[{}] Call error: {}", function, message),
                }
            }
            HostFunctionError::CryptoError { message, function, operation } => {
                write!(f, "[{}] Cryptographic error in '{}': {}", function, operation, message)
            }
            HostFunctionError::ArithmeticError { message, function, operation } => {
                write!(f, "[{}] Arithmetic error in '{}': {}", function, operation, message)
            }
        }
    }
}
...
pub fn out_of_bounds_error_with_function(offset: u32, length: u32, context: &str, function: &str) -> HostFunctionError {
    HostFunctionError::OutOfBounds {
        offset,
        length,
        message: format!("Out of bounds in {}", context),
...
pub fn invalid_parameter_error_with_function(param: &str, value: &str, context: &str, function: &str) -> HostFunctionError {
    HostFunctionError::InvalidParameter {
        param: param.to_string(),
        value: value.to_string(),
        message: format!("Invalid parameter in {}", context),
...
pub fn gas_error(message: &str, function: &str, gas_requested: Option<i64>, gas_available: Option<i64>) -> HostFunctionError {
...
pub fn call_error(message: &str, function: &str, target_address: Option<&str>) -> HostFunctionError {
